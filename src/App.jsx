...
function App() {
  const [url, setUrl] = useState(
    `https://test.uzprogers.uz/api/notes/?page=1&limit=5`
  );
  ...
  function paginationBtn(totalLocation) {
    setUrl(
      `https://test.uzprogers.uz/api/notes/?page=${totalLocation}&limit=5`
    );
  }
  ...
  async function editList(id) {
    try {
      ...
      const res = await axios.patch(
        `https://test.uzprogers.uz/api/notes/${id}`,
        {
          title: editTitle,
          content: editContent,
        }
  ...
  return (
    <div className="contsetDataainer">
      <Context.Provider value={{ editList, deleteList }}>
        <form
          onSubmit={addList}
          style={{
            margin: "40px",
